...
var imgSrc = ['https://i.imgur.com/kTwNHOH.jpg', 'https://i.imgur.com/UWb7gM0.jpg'];

store.gameOver = false;

var i = 'x';

var playerOTurn = function playerOTurn() {
  $('#playerstatus').text('Player 2 Turn');
};

var playerXTurn = function playerXTurn() {
  $('#playerstatus').text('Player 1 Turn');
};

var gameLogic = function gameLogic(event) {
  ...
  if ($('#box0').val() === $('#box1').val() && $('#box1').val() === $('#box2').val() && $('#box1').val() === 'x') {
    ...
    store.gameOver = true;
    $('section').hide(800);
  } else if (($('#box0').val() === 'x' || $('#box0').val() === 'o') && ($('#box1').val() === 'x' || $('#box1').val() === 'o') && ($('#box2').val() === 'x' || $('#box2').val() === 'o') && ($('#box3').val() === 'x' || $('#box3').val() === 'o') && ($('#box4').val() === 'x' || $('#box4').val() === 'o') && ($('#box5').val() === 'x' || $('#box5').val() === 'o') && ($('#box6').val() === 'x' || $('#box6').val() === 'o') && ($('#box7').val() === 'x' || $('#box7').val() === 'o') && ($('#box8').val() === 'x' || $('#box8').val() === 'o') && store.gameOver === false) {
...
var k = 0;
var changePic = function changePic(event) {
  // start of clicking button
  // if there is a zero or 1 end function return false
  if ($(event.target).val() === 'x' || $(event.target).val() === 'o' || store.gameOver === true) {
    return false;
  } else {
    ...
    var lastNum = idString.substr(idString.length - 1);
...
var gamesPlayedStats = function gamesPlayedStats(gamesPlayed) {
...
var updateScoreSuccess = function updateScoreSuccess(data) {
  $('#message').text('Updated!');
};

var updateScoreFailure = function updateScoreFailure() {
...
module.exports = {
  ...
  updateScoreFailure: updateScoreFailure,
  updateScoreSuccess: updateScoreSuccess,
  gamesPlayedStats: gamesPlayedStats
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var xoEvents = __webpack_require__(4);
...
var onChange = function onChange(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.onChanges(data).then(ui.changePasswordSuccess).catch(ui.changePasswordFailure);
  this.reset();
  return false;
};
...
var onSignOut = function onSignOut(event) {
  event.preventDefault();
  api.onSigningOut().then(ui.signOutSuccess).catch(ui.signOutFailure);
};
...
var onChanges = function onChanges(data) {
  return $.ajax({
...
var onSigningOut = function onSigningOut() {
  return $.ajax({
...
var store = __webpack_require__(1);
var xoEvents = __webpack_require__(4);
var backapi = __webpack_require__(15);
var ui = __webpack_require__(5);

var createGame = function createGame(event) {
  event.preventDefault();
  xoEvents.resetGame();
  backapi.createNewGame().then(ui.newGameSuccess).catch(ui.newGameFailure);
};

var updateScores = function updateScores(event) {
  var data = {
    'game': {
      'cell': {
        'index': store.index,
  ...
  event.preventDefault();
  backapi.updateScore(data).then(ui.updateScoreSuccess).catch(ui.updateScoreFailure);
};

var showIndex = function showIndex(event) {
  var data = {
    'game': {
      'id': '',
      'cells': ['', '', '', '', '', '', '', '', ''],
  ...
  event.preventDefault();
  backapi.showGame(data).then(ui.showGameSuccess).catch(ui.showGameFailure);
};
